...
import (
	...
	"github.com/Jeisson005/omni-remote-control/client-linux/internal/events"
	...
	"github.com/Jeisson005/omni-remote-control/client-linux/internal/overlay"
...
func (a *AgentClient) Start(ctx context.Context) {
	log.Printf("Starting Omni Agent for device: %s (%s)", a.cfg.DeviceID, a.cfg.DeviceName)
...
func (a *AgentClient) connectAndRun(ctx context.Context) error {
	...
	// 1. Send initial registration & static telemetry
	if err := a.sendRegistration(); err != nil {
		return err
	}

	// 2. Emit initial client_started event
	a.monitor.Emit("client_started", "info", "Agente iniciado y conectado exitosamente", map[string]interface{}{
		"version":     a.cfg.AgentVersion,
		"device_name": a.cfg.DeviceName,
	})
	go a.monitor.Start(ctx)

	// 3. Start heartbeat & dynamic metrics tickers
	...
	// Immediately send first dynamic telemetry snapshot
...
func (a *AgentClient) handleServerMessage(msg *WSMessage) {
	...
	case "command_request":
		...
		log.Printf("Executing command [%s] of type [%s]", cmdReq.ID, cmdReq.Type)
		...
		if strings.HasPrefix(cmdReq.Type, "gui_") {
			...
			res = &executor.ExecutionResult{
				ExitCode: 1,
				Error:    "unsupported command type: " + cmdReq.Type,
			}
		}
		...
		log.Printf("Finished command [%s] with status: %s (exit code: %d)", cmdReq.ID, status, res.ExitCode)
	}
}
...
func (a *AgentClient) SendStoppingEvent() {
	ev := &events.DeviceEvent{
		...
		Severity:  "info",
		Message:   "Agente deteniéndose por señal de apagado",
